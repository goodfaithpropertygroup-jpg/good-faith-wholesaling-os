...
export default function DealsPage() {
  ...
  return (
        <div className="min-h-screen bg-gray-50">
              ...
              <div className="max-w-7xl mx-auto px-4 py-6">
                      <h1 className="text-2xl font-bold mb-4">Deal Pipeline</h1>h1>
                {loading ? <p>Loading...</p>p> : (
                        <div className="overflow-x-auto bg-white rounded-lg shadow">
                                    <table className="min-w-full text-sm">
                                                  <thead><tr className="bg-gray-50 border-b">
                                                    {['Property','Status','Purchase $','ARV','Buyer','Close Date'].map(h=>(
                            <th key={h} className="px-4 py-3 text-left font-medium text-gray-500">{h}</th>th>
                          ))}
                                                  </tr>tr></thead>thead>
                                                  <tbody className="divide-y divide-gray-100">
                                                    {deals.map(d=>(
                            <tr key={d.id} className="hover:bg-gray-50">
                                                <td className="px-4 py-3 font-medium">{d.property_address}</td>td>
                                                <td className="px-4 py-3"><span className="px-2 py-0.5 bg-blue-100 text-blue-700 rounded-full text-xs">{d.status}</span>span></td>td>
                                                <td className="px-4 py-3">${d.purchase_price?.toLocaleString()}</td>td>
                                                <td className="px-4 py-3">${d.arv?.toLocaleString()}</td>td>
                                                <td className="px-4 py-3">{d.assigned_buyer}</td>td>
                                                <td className="px-4 py-3">{d.close_date}</td>td>
                            </tr>tr>
                          ))}
                                                  </tbody>tbody>
                                    </table>table>
