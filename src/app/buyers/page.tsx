                                          ...
                                          <div key={buyer.id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
                                                          ...
                                                          <div className="space-y-1 text-sm text-gray-600">
                                                              ...
                                                              {buyer.preferred_areas && (
                                                                  <p className="text-xs text-gray-500">Areas: {buyer.preferred_areas}</p>
                                                                            )}
                                                              {buyer.max_purchase_price && (
                                                                  <p className="text-xs text-gray-500">
                                                                                        Max: ${buyer.max_purchase_price.toLocaleString()}
                                                                  </p>
                                                                            )}
                                                                            <p className="text-xs text-gray-400 mt-2">
                                                                                                Deals closed: {buyer.deals_closed || 0}
                                                                                {buyer.proof_of_funds ? ' | POF verified' : ''}
                                                                            </p>
                                                          </div>
                                          </div>
                                        ))}
                                ...
                                <div className="bg-white rounded-xl p-6 w-full max-w-md shadow-xl">
                                            ...
                                            <div className="space-y-3">
                                                          ...
                                                          <input
                                                                              ...
                                                                              type="number"
                                                                              value={form.max_purchase_price}
                                                                              onChange={e => setForm(f => ({ ...f, max_purchase_price: e.target.value }))}
                                                                            />
                                                          <input
                                                                              className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm"
                                                                              placeholder="Preferred Areas (e.g. Phoenix, Scottsdale)"
                                                          ...
                                                          <label className="flex items-center gap-2 text-sm text-gray-700">
                                                                          <input
                                                                                                type="checkbox"
                                                                                                checked={form.proof_of_funds}
                                                                                                onChange={e => setForm(f => ({ ...f, proof_of_funds: e.target.checked }))}
                                                                                              />
  ...
  return (
          <div className="min-h-screen bg-gray-50 p-6">
                <div className="max-w-7xl mx-auto">
                        <div className="flex items-center justify-between mb-6">
                                  <div>
                                              <h1 className="text-2xl font-bold text-gray-900">Cash Buyers Hub</h1>h1>
                                              <p className="text-sm text-gray-500 mt-1">Manage your buyer list and track scores</p>p>
                                  </div>div>
                                  <button
                                                  onClick={() => setShowModal(true)}
                                                  className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 text-sm font-medium"
                    ...
                    {loading ? (
                        <div className="text-center py-12 text-gray-500">Loading buyers...</div>div>
                      ) : buyers.length === 0 ? (
                        <div className="text-center py-12 text-gray-500">No buyers yet. Add your first cash buyer!</div>div>
                      ) : (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {buyers.map((buyer) => (
                                          <div key={buyer.id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
                                                          ...
                                                          <div className="space-y-1 text-sm text-gray-600">
                                                              {buyer.email && <p>{buyer.email}</p>p>}
                                                              {buyer.phone && <p>{buyer.phone}</p>p>}
                                                              {buyer.preferred_areas && (
                                                                  <p className="text-xs text-gray-500">Areas: {buyer.preferred_areas}</p>p>
                                                                            )}
                                                              {buyer.max_purchase_price && (
                                                                  <p className="text-xs text-gray-500">
                                                                                        Max: ${buyer.max_purchase_price.toLocaleString()}
              ...
              {showModal && (
                      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                                <div className="bg-white rounded-xl p-6 w-full max-w-md shadow-xl">
                                            <h2 className="text-lg font-semibold mb-4">Add Cash Buyer</h2>h2>
                                            <div className="space-y-3">
                                                          <div className="grid grid-cols-2 gap-3">
                                                                          <input
                                                                                                className="border border-gray-200 rounded-lg px-3 py-2 text-sm"
