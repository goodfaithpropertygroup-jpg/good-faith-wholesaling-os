          ...
          <div className="min-h-screen bg-gray-50 p-6">
                <div className="max-w-7xl mx-auto">
                        <div className="flex items-center justify-between mb-6">
                                  <div>
                                              <h1 className="text-2xl font-bold text-gray-900">Lead CRM</h1>h1>
                                              <p className="text-sm text-gray-500 mt-1">Track and manage all your leads</p>p>
                                  </div>div>
                        ...
                        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                            {loading ? (
                          <div className="p-8 text-center text-gray-500">Loading leads...</div>div>
                        ) : leads.length === 0 ? (
                          <div className="p-8 text-center text-gray-500">No leads found. Add your first lead!</div>div>
                        ) : (
                                        ...
                                        <tbody className="divide-y divide-gray-50">
                                            {leads.map(lead => (
                                                <tr key={lead.id} className="hover:bg-gray-50 transition-colors">
                                                                    <td className="px-4 py-3 font-medium text-gray-900">{lead.first_name} {lead.last_name}</td>td>
                                                                    <td className="px-4 py-3 text-gray-600">
                                                                                          <div>{lead.email}</div>div>
                                                                                          <div className="text-xs text-gray-400">{lead.phone}</div>div>
                                                                    </td>td>
                                                                    <td className="px-4 py-3 text-gray-600">
                                                                                          <div>{lead.address}</div>div>
                                                                                          <div className="text-xs text-gray-400">{lead.city}, {lead.state} {lead.zip}</div>div>
                                                                    </td>td>
                                                                    ...
                                                                    <td className="px-4 py-3 text-gray-600 capitalize">{lead.source?.replace('_', ' ')}</td>td>
                                                                    <td className="px-4 py-3 text-gray-600">{lead.asking_price ? `$${lead.asking_price.toLocaleString()}` : '-'}</td>td>
                                                                    <td className="px-4 py-3 text-gray-400 text-xs">{new Date(lead.created_at).toLocaleDateString()}</td>td>
                                                </tr>tr>
                                              ))}
                                        </tbody>tbody>
                          </table>table>
              ...
              {showModal && (
                      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                                <div className="bg-white rounded-xl p-6 w-full max-w-lg shadow-xl max-h-screen overflow-y-auto">
                                            <h2 className="text-lg font-semibold mb-4">Add New Lead</h2>h2>
                                            <div className="space-y-3">
                                                          ...
                                                          <div className="grid grid-cols-2 gap-3">
                                                                          <select className="border border-gray-200 rounded-lg px-3 py-2 text-sm" value={form.status} onChange={e => setForm(f => ({ ...f, status: e.target.value }))}>
                                                                                            ...
                                                                                            <option value="follow_up">Follow Up</option>option>
                                                                                            <option value="under_contract">Under Contract</option>option>
                                                                                            <option value="closed">Closed</option>option>
                                                                                            <option value="dead">Dead</option>option>
                                                                          </select>select>
                                                                          <select className="border border-gray-200 rounded-lg px-3 py-2 text-sm" value={form.source} onChange={e => setForm(f => ({ ...f, source: e.target.value }))}>
                                                                                            ...
                                                                                            <option value="website">Website</option>option>
                                                                          </select>select>
                                                          </div>div>
                                            ...
                                            <div className="flex gap-3 mt-5">
                                                          <button onClick={() => setShowModal(false)} className="flex-1 border border-gray-200 text-gray-700 px-4 py-2 rounded-lg text-sm hover:bg-gray-50">Cancel</button>button>
                                                          <button onClick={addLead} disabled={!form.first_name || !form.address} className="flex-1 bg-blue-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-blue-700 disabled:opacity-50">Add Lead</button>button>
                                            </div>div>
                                </div>div>
                      </div>div>
                )}
